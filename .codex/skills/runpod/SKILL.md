---
name: runpod
description: Launch, monitor, retrieve, and safely terminate independent GPU experiment Pods on RunPod using the official REST/GraphQL APIs. Use when Codex needs to validate RunPod access, estimate cost, run a one-Pod smoke test, or launch a YAML/JSON experiment matrix while preserving artifacts and avoiding leaked credentials.
---

# RunPod experiments

Use `scripts/runpod_matrix.py` from the target repository with its local
...
- A repository URL and commit. Omit either only when running in a Git checkout,
  where the script resolves `origin` and `HEAD`.
...
1. Run `check` and `validate`, then request an API-backed `estimate`.
...
3. With explicit authorization for spend, launch one short smoke job. The
   default workflow retrieves artifacts and terminates every recorded Pod even
   on a job, transfer, or partial-provisioning failure.
...
.venv/bin/python .codex/skills/runpod/scripts/runpod_matrix.py launch --matrix PATH --estimate-hours HOURS --max-parallel N
